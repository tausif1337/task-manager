        ...
        {tasks.length === 0 ? (
          ...
          tasks.map((task, index) => (
            <div key={index} className="flex justify-between items-center p-3 mb-2 bg-gray-100 rounded">
              <span onClick={() => toggleTask(index)} className={`flex-1 cursor-pointer ${task.completed ? "line-through text-gray-400" : ""}`}>{task.text}</span>
              <button onClick={() => deleteTask(index)} className="ml-2 px-3 py-1 bg-red-500 text-white rounded">Delete</button>
            </div>
          ))
        )}
      </div>
